...
export function EcoRanking({ rows, rank, total }: { rows: EcoRankingRow[]; rank: number; total: number }) {
  ...
  return (
    <div className="flex h-full flex-col p-5 lg:border-l lg:border-slate-200 lg:pl-5 dark:lg:border-slate-800">
      ...
      <div className="mt-3 flex gap-2 px-3 text-[10px] font-semibold text-slate-500 dark:text-slate-400">
        <span className="w-5 shrink-0" />
        <span className="max-w-[180px] flex-1">{t.carbon.rankColVessel}</span>
        <span className="w-28 shrink-0 text-right">{t.carbon.rankColSaved}</span>
        <span className="w-20 shrink-0 text-right">{t.carbon.rankColValue}</span>
      </div>
      ...
      <div className="mt-1.5 space-y-1.5">
        {rows.map((row, i) => (
          ...
          >
            ...
            <span
              className={cn(
                'max-w-[180px] flex-1 truncate text-sm',
            ...
            <span className="w-28 shrink-0 text-right font-mono text-sm font-semibold">
              {formatSignedPct(row.co2SavedPct, t.carbon.pctSaved, t.carbon.pctIncreased)}
            </span>
            <span className="w-20 shrink-0 text-right font-mono text-sm font-semibold text-[#10B981]">
